...
| Category         | Instruction    | Bytecode  | Signature / Purpose                                       | Example              |
|:-----------------|:---------------|:----------|:----------------------------------------------------------|:---------------------|
| **Data Ingress** | `PUSH_CONST`   | `0x01`    | **Stream Arg**: 1 (Value). Pushes a literal float.        | `2.5`, `360.0`       |
|                  | `GET_TIME`     | `0x02`    | **0 Args**. Pushes smooth continuous seconds.             | Continuous motion.   |
|                  | `GET_STATE`    | `0x03`    | **Stream Arg**: 1 (Index). Pushes value from `fact[n]`.   | Reactor sensor data. |
|                  | `GET_SIGNAL`   | `0x04`    | **Stream Arg**: 1 (Index). Pushes value from `signal[n]`. | Syncing layers.      |
| **Core Math**    | `ADD`, `SUB`   | `0x10-11` | **Stack Args**: 2 `(a, b)`. Addition/Subtraction.         | `x + offset`         |
|                  | `MUL`, `DIV`   | `0x12-13` | **Stack Args**: 2 `(a, b)`. Multiplication/Division.      | `time * speed`       |
|                  | `MOD`          | `0x14`    | **Stack Args**: 2 `(a, b)`. Remainder/Looping.            | `time % loop`        |
|                  | `SIN`, `COS`   | `0x20-21` | **Stack Arg**: 1 `(rad)`. Trigonometric motion.           | Circular orbits.     |
|                  | `ABS`, `SQRT`  | `0x22-23` | **Stack Arg**: 1 `(v)`. Absolute/Square Root.             | Distance math.       |
| **Macros**       | `REMAP`        | `0x30`    | **Stack Args**: 5 `(v, iL, iH, oL, oH)`. Range mapping.   | `Batt` -> `Alpha`    |
|                  | `CLAMP`        | `0x31`    | **Stack Args**: 3 `(v, min, max)`. Range constraint.      | Safe scale limits.   |
|                  | `STEP`         | `0x32`    | **Stack Args**: 2 `(edge, v)`. 0.0 or 1.0 trigger.        | Goal thresholds.     |
|                  | `LERP`         | `0x33`    | **Stack Args**: 3 `(a, b, t)`. Linear interpolation.      | Point-to-point move. |
|                  | `OSCILLATE`    | `0x34`    | **Stack Args**: 4 `(center, amp, speed, phase)`.          | Swaying foliage.     |
| **Logic**        | `IF_GT`        | `0x40`    | **Stack Args**: 4 `(v, thresh, then, else)`. Ternary.     | Decision making.     |
|                  | `AND`, `OR`    | `0x41-42` | **Stack Args**: 2. (Result: 1.0 if True, else 0.0).       | Combined triggers.   |
| **Atmosphere**   | `NOISE`        | `0x50`    | **Stack Arg**: 1 (Seed). Deterministic randomness.        | Water ripples.       |
|                  | `MIX_OKLAB`    | `0x51`    | **Stack Args**: 3 `(colorA, colorB, t)`. Blending.        | Vibrant sunsets.     |
| **Motion**       | `EASE_IN_OUT`  | `0x60`    | **Stack Arg**: 1 `(t)`. Smooth start/stop easing.         | Cinematic motion.    |
|                  | `EASE_BACK`    | `0x61`    | **Stack Arg**: 1 `(t)`. Overshooting bounce.              | Bending branches.    |
|                  | `EASE_ELASTIC` | `0x62`    | **Stack Arg**: 1 `(t)`. Spring-like oscillation.          | Notification pops.   |
