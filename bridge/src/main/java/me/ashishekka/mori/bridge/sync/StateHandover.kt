...
object StateHandover {
    ...
    fun sync(from: WorldState, to: MoriEngineState) {
        ...
        if (to.surfaceWidth > 0 && to.surfaceHeight > 0) {
            val width = to.surfaceWidth.toFloat()
            val height = to.surfaceHeight.toFloat()

            to.setFieldValue(MoriEngineStateIndices.FACT_ASPECT_RATIO, height / width)
            to.setFieldValue(MoriEngineStateIndices.FACT_IS_LANDSCAPE, if (width > height) 1.0f else 0.0f)
            to.setFieldValue(MoriEngineStateIndices.FACT_FIELD_RATIO, width / height)
        }
        }
        }
